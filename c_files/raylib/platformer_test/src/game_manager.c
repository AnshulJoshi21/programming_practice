...
void game_manager_init(GameManager *gm)
{
    ...
    gm->camera = (Camera2D){.offset = (Vector2){GetScreenWidth() / 2.0f,
...
void game_manager_draw(const GameManager *gm, const Texture2D sheet)
{
    ...
    player_draw(&gm->player, sheet);

    EndMode2D();

    DrawText(TextFormat("Speed: %f", gm->player.h_speed), 10, 10, 20, BLACK);
}
...
static void camera_update(GameManager *gm)
{
    ...
    gm->camera.target.x = fmaxf(half_w,
                                fminf(gm->player.dest.x, MAP_SIZE - half_w));
    gm->camera.target.y = fmaxf(half_h,
                                fminf(gm->player.dest.y, MAP_SIZE - half_h));
}
