...
void game_manager_init(GameManager* gm) {
    ...
    // spawn orbital
    bullet_manager_spawn(
        &gm->bullet_manager,
        (BulletInitConfig){
            1, BULLET_BEHAVIOUR_ORBITAL, BULLET_TYPE_ORBITAL_ORBS, {.orbital = {&gm->player}}});
}
...
Vector2 get_player_target(GameManager* gm) {
    assert(gm);
...
void game_manager_update(GameManager* gm, const float dt) {
    ...
    enemy_manager_update(&gm->enemy_manager, dt, &gm->player);
    ...
    // spawn
    // projectile
    const Vector2 target_pos = get_player_target(gm);
    if (interval_timer_tick(&gm->player.projectile_timer, dt)) {
        bullet_manager_spawn(
            &gm->bullet_manager,
            (BulletInitConfig){
                1,
                ...
                {.projectile = {(Vector2){gm->player.rect.x, gm->player.rect.y}, &target_pos}}});
    }
}
...
static void draw_background_grid(void) {
    ...
    const Color color      = palette.beige;
